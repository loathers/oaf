...
async function spadeItems(interaction: ChatInputCommandInteraction) {
  ...
  await kolClient.useFamiliar(SpadingFamiliars.GHOST);
  ...
  await kolClient.useFamiliar(SpadingFamiliars.DEFAULT);
  ...
  for (const spadeData of data) {
    ...
    const familiar = await testEquipmentAgainstCurrentFamiliar(spadeData.id);
    if (familiar !== null) {
